...
def predict_rating_for_pair(user, business, business_sets, user_sets, business_means, user_means, overall_avg, neighbor_limit=75):
    if business not in business_sets:
        if user in user_sets:
            return user_means.get(user, overall_avg)
        else:
            return (user_means.get(user, overall_avg) + business_means.get(business, overall_avg)) / 2 if business in business_means else overall_avg
    ...
    for b_i, rating in user_ratings.items():
        if b_i == business or b_i not in business_sets:
            continue
        sim = compute_pearson(business_sets[business], business_sets[b_i])
        if sim == 0:
            continue
        sim *= 0.8 if sim < 0 else 1.2
        normalized_rating = rating - (user_means.get(user, overall_avg) + business_means.get(b_i, overall_avg) - overall_avg)
    ...
    denominator = sum(abs(sim) for sim, _ in neighbor_list)

    baseline = (user_means.get(user, overall_avg) + business_means.get(business, overall_avg)) / 2
    prediction = baseline + numerator / denominator if denominator != 0 else baseline
    return min(max(prediction, 1.0), 5.0)

def main(train_file, test_file, output_file):
    spark = SparkSession.builder.appName("task2_1_optimized").getOrCreate()
